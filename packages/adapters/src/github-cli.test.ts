...
describe("GitHub CLI code host adapter", () => {
  ...
  it("fails mergeability reads when gh fails", async () => {
    ...
    await expect(adapter.getPullRequestMergeability(pr.id)).rejects.toThrow(/gh pr view failed \(1\): not found/i);
  });
  ...
  it("fails mergeability reads when gh returns invalid JSON", async () => {
    ...
    await expect(adapter.getPullRequestMergeability(pr.id)).rejects.toThrow(/could not parse pull request mergeability json/i);
  });
});
