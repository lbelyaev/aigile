...
describe("runtime config", () => {
  it("loads provider-neutral ACP runtime assignments from JSON", () => {
    const config = loadRuntimeConfigFromJson(JSON.stringify({
      runtimes: [{
  ...
  it("builds a role runtime registry from config", () => {
    const registry = runtimeConfigToRegistry(loadRuntimeConfigFromJson(JSON.stringify({
      runtimes: [{
        id: "checker-runtime",
        transport: "stdio",
  ...
  it("rejects invalid runtime config", () => {
    expect(() => loadRuntimeConfigFromJson(JSON.stringify({
